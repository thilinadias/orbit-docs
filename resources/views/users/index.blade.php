<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <a href="{{ route('users.create') }}" class="px-4 py-2 text-sm font-bold leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                Add New User
            </a>
        </div>
    </x-slot>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-gray-800">
        <form method="GET" action="{{ route('users.index') }}" class="flex flex-col md:flex-row gap-4 mb-6">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search users by name or email..." class="w-full text-sm rounded-md border-gray-300 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600" />
            </div>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center text-sm">
                                <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                    <img class="object-cover w-full h-full rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&color=7F9CF5&background=EBF4FF" alt="" loading="lazy" />
                                ...
                                @foreach($user->organizations as $org)
                                    <span class="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 rounded text-nowrap">
                                        ...
                                        @if($org->pivot->role_id)
                                            <span class="text-xs opacity-75">({{ \App\Models\Role::find($org->pivot->role_id)?->name }})</span>
                                        @endif
                                    </span>
                                @endforeach
                            </div>
